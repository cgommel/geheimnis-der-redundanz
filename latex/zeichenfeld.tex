...
\NewDocumentCommand{\@zfCode}{m}{%
  \begin{tikzpicture}[x=1mm, y=1mm, baseline=0]
    ...
    %
    % Modul-Hilfsraster: durchgezogene vertikale Linien an jeder
    % Modul-Grenze. Linienbreite bewusst über der Aliasing-Schwelle
    % (~0.4pt), damit alle 96 Linien beim Drucken gleich dick wirken
    % und Greta die Module abzählen kann, ohne das Lineal anzulegen.
    \foreach \x in {0,1,...,95}{%
      \draw[gray!55, line width=0.4pt] (\x,-3) -- (\x,33);%
    }
    % Obere und untere Begrenzung der Daten-Strichhöhe.
    \draw[gray!70, line width=0.5pt] (0,0)  -- (95,0);
    \draw[gray!70, line width=0.5pt] (0,33) -- (95,33);
    %

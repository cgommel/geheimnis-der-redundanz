...
\begin{tikzpicture}[x=0.9cm, y=0.8cm, font=\small]
  % ── Farben ──────────────────────────────────────────────────
  % B1=blau, B2=orange, Kratzer=rot/transparent, beschädigt=grau

  % ── Ohne Interleaving ───────────────────────────────────────
  \node[anchor=west, font=\small\bfseries] at (-0.05, 3.5)
    {Ohne Interleaving};

  % 12 Felder: 0-5 = B1 (blau), 6-11 = B2 (orange)
  \foreach \i in {0,...,5} {
    \fill[blue!35, draw=black!30] (\i, 2.2) rectangle +(0.88, 0.65);
    \node[font=\scriptsize] at (\i+0.44, 2.52) {B1};
  }
  \foreach \i in {6,...,11} {
    \fill[orange!45, draw=black!30] (\i, 2.2) rectangle +(0.88, 0.65);
    \node[font=\scriptsize] at (\i+0.44, 2.52) {B2};
  }
  % Kratzer überdeckt Felder 2-6 (5 Felder, davon 4×B1, 1×B2)
  \fill[red!25] (2, 2.15) rectangle (7, 2.90);
  \draw[red, thick] (2, 2.15) rectangle (7, 2.90);
  % Pfeil + Label
  \draw[red, thick, <->] (2, 3.05) -- (7, 3.05)
    node[midway, above, font=\scriptsize\bfseries, red] {Kratzer};
  % Ergebnis-Annotation
  \node[anchor=west, font=\scriptsize, blue!70!black] at (0, 1.85)
    {Block 1: \textbf{4 Fehler} — zu viele! $\Rightarrow$ FEHLER};
  \node[anchor=west, font=\scriptsize, orange!70!black] at (6.2, 1.85)
    {Block 2: 1 Fehler $\checkmark$};

  % ── Mit Interleaving ────────────────────────────────────────
  \node[anchor=west, font=\small\bfseries] at (-0.05, 1.4)
    {Mit Interleaving};

  % Abwechselnd B1/B2: 0,2,4,6,8,10 = B1 ; 1,3,5,7,9,11 = B2
  \foreach \i/\bl in {0/1,1/2,2/1,3/2,4/1,5/2,6/1,7/2,8/1,9/2,10/1,11/2} {
    \ifnum\bl=1
      \fill[blue!35, draw=black!30] (\i, 0.2) rectangle +(0.88, 0.65);
      \node[font=\scriptsize] at (\i+0.44, 0.52) {B1};
    \else
      \fill[orange!45, draw=black!30] (\i, 0.2) rectangle +(0.88, 0.65);
      \node[font=\scriptsize] at (\i+0.44, 0.52) {B2};
    \fi
  }
  % Gleicher Kratzer
  \fill[red!25] (2, 0.15) rectangle (7, 0.90);
  \draw[red, thick] (2, 0.15) rectangle (7, 0.90);
  \draw[red, thick, <->] (2, 1.05) -- (7, 1.05)
    node[midway, above, font=\scriptsize\bfseries, red] {gleicher Kratzer};
  % Ergebnis-Annotation
  \node[anchor=west, font=\scriptsize, blue!70!black] at (0, -0.15)
    {Block 1: \textbf{2 Fehler} $\checkmark$};
  \node[anchor=west, font=\scriptsize, orange!70!black] at (4.5, -0.15)
    {Block 2: \textbf{3 Fehler} $\checkmark$};
  \node[anchor=west, font=\scriptsize, green!50!black] at (0, -0.55)
    {→ beide Blöcke reparieren sich selbst (ECC-Grenze: 3 Fehler/Block)};
\end{tikzpicture}
\end{center}

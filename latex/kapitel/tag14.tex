...
Du hast jetzt einen kompletten Datamatrix-Encoder von ASCII bis PNG —
in einem einzigen Skript, etwa 100 Zeilen lang, ohne irgendeine
Encoding-Bibliothek (nur Pillow für die Pixel und die
Tag-13-Platzierungslogik). Das ist mehr, als die meisten Software-
Entwickler je über Datamatrix gesehen haben.

\subsection*{Was am Skript noch unschön ist}

\begin{itemize}
\item Alles steht auf Top-Level. Es gibt keine Funktionen, also
  auch keinen Weg, mit anderem Text als \texttt{GRETA} zu encoden,
  ohne die Konstante \texttt{TEXT} oben zu ändern und das Skript
  neu zu starten.
\item Die GF(2$^8$)-Multiplikation und der RS-Encoder sind inline
  — das war heute willkommen (Wiederholung), aber für jeden
  weiteren Encoder müsste man sie wieder copy-pasten.
\item Wir können nur 12$\times$12. Andere Symbol-Größen (14$\times$14,
  16$\times$16, \dots) brauchen andere Codeword-Anzahlen, mehr
  ECC-Blöcke, andere Padding-Regeln.
\end{itemize}

\subsection*{Vorschau Tag 15}

Wir machen aus dem Skript eine \texttt{DatamatrixEncoder}-Klasse —
sauber gekapselt mit Methoden für jeden Schritt. Damit lässt sich
der Encoder wie ein Werkzeug benutzen:

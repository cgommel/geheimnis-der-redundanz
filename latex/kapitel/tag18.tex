...
% =============================================================
\section{Reflexion — das Ende einer Reise \normalfont(\textit{≈ 15 min})}
% =============================================================

Greta hält inne. Zwei Wochen sind vergangen.

Am ersten Tag war ein EAN-13-Strichcode — zehn schwarze Balken, eine
einzige Prüfziffer, kaum mehr als eine Quersumme. Jetzt sitzt sie vor
einem Datamatrix-Symbol, das ihren eigenen Namen trägt, auch wenn man
ihn mit dem Filzstift halb überschmiert hat; das falsche Bits repariert,
bevor sie überhaupt bemerkt werden; das C40-Segmente und Digit-Paare
selbst erkennt und einpackt.

\textbf{Was sie gelernt hat:}

\begin{itemize}
\item \textbf{Etappen 1–2}: Prüfziffern — EAN-13, ISBN-10, Luhn,
  Hamming-Distanz. Fehler erkennen, ohne zu korrigieren.
\item \textbf{Etappen 3–4}: Hamming-Code, SECDED, CRC — echtes
  Korrigieren, Bündelfehler, Interleaving auf Bitebene.
\item \textbf{Etappen 5–6}: Endliche Körper $\mathrm{GF}(2^n)$ und
  Polynome darüber — die Mathematik, auf der Reed-Solomon aufbaut.
\item \textbf{Etappen 7–8}: Reed-Solomon-Encoder und -Decoder — zuerst
  von Hand über $\mathrm{GF}(2^3)$, dann in Python mit \texttt{reedsolo}.
\item \textbf{Etappen 9–10}: EAN-13 von Hand und mit Pillow —
  Strichbreiten, Wachzone, Prüfziffer.
\item \textbf{Etappen 11–14}: Datamatrix von Grund auf — ASCII-Encoding,
  RS-Prüfbytes, Datenplatzierung, erster scannbarer Code.
\item \textbf{Etappen 15}: Klassen und Vererbung — \texttt{Barcode} als
  Basis, \texttt{DatamatrixEncoder} als Spezialisierung.
\item \textbf{Etappen 16}: Decoder — inverse Datenplatzierung,
  Fehlerlokalisation, Stress-Test.
\item \textbf{Etappe 17}: Große Codes und Interleaving — Symbole bis
  64×64, RS-Blöcke verschachteln.
\item \textbf{Etappe 18}: Modi — Digit-Paar, C40, automatischer
  Moduswechsler. Gleicher Inhalt, kleines Symbol.
\end{itemize}

\begin{minted}{python}
e = DatamatrixEncoderModi("GRETA")
e.save("greta_final.png")
\end{minted}

Das Symbol scannt. Es ist 12×12 — das kleinste, das den Namen fasst.
Praktikum vorbei.

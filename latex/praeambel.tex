...
% "a → ä, "n verschluckt sich etc.). Das produziert in Code-Snippets
% (\texttt{bild.save("name.png")}) absurde Verschmelzungen.
% \shorthandoff{"} allein reicht nicht — Babel reaktiviert sie bei
% jedem Sprachwechsel. Daher zusätzlich \languageshorthands{none}
% in den ngerman-Hook.
\shorthandoff{"}
...
% --- Fallback für fehlende Glyphen ---
% Source Serif/Sans/Code-Pro decken nicht jeden Unicode-Math-Glyph
% ab (z. B. ⊕ U+2295, ⌊⌋ U+230A/B, ↔ U+2194). Wenn solche Zeichen
% direkt im Quelltext stehen — vor allem in verbatim/minted-Blöcken,
% wo math-Mode nicht greift — würden sie als „Tofu" rendern.
% STIX Two Math hat sie alle und ist sowieso schon geladen; per
% newunicodechar lenken wir die nackten Unicode-Zeichen darauf um.
\usepackage{newunicodechar}
\newfontfamily\fallbacksym{STIXTwoMath-Regular.otf}[Path=fonts/stix-two/]
...
% --- pdfpages: erlaubt das Einbinden externer PDFs (Werkstattbogen
% in Etappe 9, weitere Beilagen in späteren Etappen). ---
\usepackage{pdfpages}

% --- graphicx: für Pillow-Beispielbilder ab Etappe 10
% (von pdfpages indirekt schon geladen, hier nur Klarheit).
\usepackage{graphicx}

% --- EAN-13-Encoding + Renderer (Etappe 9 ff.) ---
% Eigener parametrisierter Renderer mit Lookup-Tabellen, statt GS1 —
% damit haben wir Kontrolle über jedes einzelne Modul (für die
% Zeichenvorlage in Etappe 9 brauchen wir das, um die Module ins
% Hilfsraster einzeichnen zu können).
\usepackage{xstring}
...
% --- Worttrennungen für die langen Komposita, die sich im Glossar
% und in der schmalen Aufgabenbox-Spalte sonst nicht brechen lassen.
\hyphenation{%
  Ein-zel-feh-ler-Kor-rek-tur
  Dop-pel-feh-ler-Er-ken-nung
  Pari-täts-bit
  Pari-täts-mu-ster
  Wie-der-ho-lungs-code
  Min-dest-di-stanz
  Hamming-Schran-ke
  Auslö-schungs-De-co-der
  Auslö-schungs-Po-si-tio-nen
  Kor-rek-tur-bits
  Pseudo-Kor-rek-tur
  Hellzo-ne
  Da-ten-mo-du-le
